...
async fn main() -> Result<()> {
    ...
    info!("Subscribing to market {} with depth {}...", market_id, orderbook_depth);
    streaming_client.subscribe_to_market(market_id.to_string(), orderbook_depth).await?;
...
fn print_orderbook_summary(market_id: &str, market_books: &HashMap<String, Orderbook>) {
    println!("\n{}", "=".repeat(80));
    println!("Market ID: {} | Time: {}", market_id, chrono::Local::now().format("%H:%M:%S"));
    println!("{}", "-".repeat(80));
    ...
    for (selection_id, orderbook) in market_books.iter() {
        ...
        println!("{:^10} {:^9} | {:^9} {:^10}", "Price", "Size", "Price", "Size");
        println!("{}", "-".repeat(41));
        ...
        if let (Some(best_bid), Some(best_ask)) = (orderbook.get_best_bid(), orderbook.get_best_ask()) {
            let spread = best_ask.price - best_bid.price;
            println!("\nSpread: {:.2} | Mid: {:.2}", spread, (best_bid.price + best_ask.price) / 2.0);
        }
    }
}